...
const state = {
  ...
  categories:{LIVE:null,VOD:null,SERIES:null}, streams:new Map(), seriesInfo:new Map(),
  ...
  player:null, playerTarget:null, sourceIndex:0, retryCount:0, retryTimer:null,
  wakeLock:null, controlsTimer:null, progressTimer:null, lastApiAt:0,
};
...
function setView(id){['loginView','mainView','playerView'].forEach(v=>$(v).classList.toggle('active',v===id))}
...
async function api(action,extra={}){
  const res=await fetch(apiUrl(action,extra),{cache:'no-store',mode:'cors'});
  if(!res.ok)throw new Error(`HTTP ${res.status}`);
  return res.json();
}

async function login(server,user,pass,proxy,remember){
  state.server=normalizeServer(server);state.user=user.trim();state.pass=pass;state.proxy=normalizeServer(proxy);
...
async function getCategories(type,force=false){
  if(!force&&state.categories[type])return state.categories[type];
  const action=type==='LIVE'?'get_live_categories':type==='VOD'?'get_vod_categories':'get_series_categories';
  const list=await api(action);state.categories[type]=Array.isArray(list)?list:[];return state.categories[type];
}
async function getStreams(type,cat='0',force=false){
  const key=`${type}:${cat}`;const cached=state.streams.get(key);
  if(!force&&cached&&Date.now()-cached.at<5*60*1000)return cached.data;
...
async function openContent(type,force=false){
  state.currentType=type;state.currentCategory='0';activateTab('home');
  const c=$('mainContent');c.innerHTML='<div class="loading">Loading…</div>';
  try{
    const [cats,items]=await Promise.all([getCategories(type,force),getStreams(type,'0',force)]);state.currentItems=items;renderContent(type,cats,items);
  }catch(e){renderApiError(e,()=>openContent(type,true))}
}
function renderContent(type,cats,items){
  const c=$('mainContent');
  ...
  const chips=$('chips');[{category_id:'0',category_name:'All'},...cats].forEach(cat=>{const b=document.createElement('button');b.className='chip'+(cat.category_id==='0'?' active':'');b.textContent=cat.category_name;b.onclick=async()=>{document.querySelectorAll('.chip').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.currentCategory=String(cat.category_id);$('contentGrid').innerHTML='<div class="loading">Loading…</div>';try{state.currentItems=await getStreams(type,state.currentCategory);drawItems(type,state.currentItems)}catch(e){renderApiError(e,()=>openContent(type,true))}};chips.appendChild(b)});
...
function drawItems(type,items){
  ...
  const frag=document.createDocumentFragment();items.slice(0,1200).forEach(it=>{
    const b=document.createElement('div');b.className='card '+(type==='LIVE'?'live-card':'');b.setAttribute('role','button');b.tabIndex=0;
    const poster=itemPoster(it);b.innerHTML=`<div class="poster">${poster?`<img loading="lazy" src="${escapeHtml(poster)}" alt="">`:'<div class="fallback-logo">A</div>'}</div><button class="fav-btn" aria-label="Favorite">${isFav(type,it)?'♥':'♡'}</button><div class="name">${escapeHtml(itemName(it))}</div>`;
...
async function openSeries(series){
  ...
  try{
    ...
    c.innerHTML=`<div class="section-head"><button class="back-link" id="seriesBack">‹ Series</button><h2>${escapeHtml(itemName(series))}</h2><span></span></div><div id="seasons"></div>`;$('seriesBack').onclick=()=>openContent('SERIES');
...
function playItem(type,it){const urls=type==='LIVE'?liveUrls(it):vodUrls(it);const key=`${type}:${itemId(type,it)}`;startPlayer({type,title:itemName(it),icon:itemPoster(it),itemId:itemId(type,it),ext:it.container_extension||'mp4',urls,seekable:type!=='LIVE',resumeKey:key,start:Number(state.resume[key]?.time||0)})}
function playEpisode(series,ep){const key=`SERIES:${ep.id}`;startPlayer({type:'SERIES',title:`${itemName(series)} · Episode ${ep.episode_num??''}`,icon:itemPoster(series),itemId:String(ep.id),ext:ep.container_extension||'mp4',urls:epUrls(ep),seekable:true,resumeKey:key,start:Number(state.resume[key]?.time||0)})}

async function startPlayer(target){
  const v=$('video');state.playerTarget=target;state.sourceIndex=0;state.retryCount=0;setView('playerView');applyScale();showControls();$('playerTitle').textContent=target.title;$('seekWrap').classList.toggle('hidden',!target.seekable);$('retryBtn').classList.add('hidden');$('playerStatus').textContent='Loading…';
...
function loadSource(index,start=0){
  const t=state.playerTarget,v=$('video');if(!t||index>=t.urls.length){fatalPlayerError('Stream failed');return}state.sourceIndex=index;state.retryCount=0;$('retryBtn').classList.add('hidden');$('playerStatus').textContent=index?'Trying fallback stream…':'Loading…';v.pause();v.removeAttribute('src');v.load();v.src=t.urls[index];v.load();if(start>0){v.addEventListener('loadedmetadata',function s(){v.removeEventListener('loadedmetadata',s);try{v.currentTime=Math.min(start,Math.max(0,(v.duration||start)-5))}catch{}},{once:true})}v.play().catch(()=>{$('playerStatus').textContent='Tap play to start'});
}
function attachPlayerEvents(){
  const v=$('video');if(v._aymanBound)return;v._aymanBound=true;
  v.addEventListener('playing',()=>{$('playerStatus').textContent='';$('playPauseBtn').textContent='❚❚';state.retryCount=0;startProgressSaver()});
  v.addEventListener('pause',()=>{$('playPauseBtn').textContent='▶';persistResume()});
  v.addEventListener('waiting',()=>{$('playerStatus').textContent='Buffering…'});
  v.addEventListener('timeupdate',()=>{if(!state.playerTarget?.seekable)return;const d=v.duration||0;if(d>0){$('seekBar').value=String(Math.floor((v.currentTime/d)*1000));$('currentTime').textContent=fmtTime(v.currentTime);$('durationTime').textContent=fmtTime(d)}});
  v.addEventListener('ended',()=>{clearResume();closePlayer()});
  v.addEventListener('error',()=>handlePlaybackError());
  v.addEventListener('click',()=>{if($('playerControls').classList.contains('visible'))hideControls();else showControls()});
  document.addEventListener('visibilitychange',async()=>{if(document.visibilityState==='visible'&&state.playerTarget){await acquireWakeLock();if(v.paused&&!v.ended)v.play().catch(()=>{})}});
}
function handlePlaybackError(){
  const t=state.playerTarget;if(!t)return;
...
function fatalPlayerError(msg){$('playerStatus').textContent=msg;$('retryBtn').classList.remove('hidden');showControls()}
...
function cycleScale(){state.scale=state.scale==='fit'?'fill':state.scale==='fill'?'zoom':'fit';applyScale();showControls()}
function showControls(){const c=$('playerControls');c.classList.add('visible');clearTimeout(state.controlsTimer);state.controlsTimer=setTimeout(()=>{if(!$('video').paused)hideControls()},3000)}
function hideControls(){$('playerControls').classList.remove('visible')}
...
async function closePlayer(){persistResume();clearInterval(state.progressTimer);clearTimeout(state.retryTimer);const v=$('video');v.pause();v.removeAttribute('src');v.load();await releaseWakeLock();try{if(document.fullscreenElement)await document.exitFullscreen()}catch{}try{screen.orientation?.unlock?.()}catch{}state.playerTarget=null;setView('mainView')}
function updateRotateHint(){$('rotateHint').classList.toggle('hidden',matchMedia('(orientation: landscape)').matches)}
addEventListener('orientationchange',updateRotateHint);addEventListener('resize',updateRotateHint);

function renderContinue(){activateTab('continue');const c=$('mainContent');const list=Object.entries(state.resume).filter(([,r])=>r&&r.time>10).sort((a,b)=>b[1].timestamp-a[1].timestamp);c.innerHTML='<div class="section-head"><h2>Continue Watching</h2></div><div class="grid" id="continueGrid"></div>';const g=$('continueGrid');if(!list.length){g.innerHTML='<div class="empty">Nothing to resume yet</div>';return}list.forEach(([key,r])=>{const b=document.createElement('button');b.className='card';const pct=r.duration?Math.min(100,(r.time/r.duration)*100):0;b.innerHTML=`<div class="poster">${r.icon?`<img src="${escapeHtml(r.icon)}" alt="">`:'<div class="fallback-logo">A</div>'}</div><div class="progress"><span style="width:${pct}%"></span></div><div class="name">${escapeHtml(r.name)}</div>`;b.onclick=()=>{const urls=r.type==='VOD'?[`${state.server}/movie/${encodeURIComponent(state.user)}/${encodeURIComponent(state.pass)}/${r.itemId}.${r.ext||'mp4'}`]:[`${state.server}/series/${encodeURIComponent(state.user)}/${encodeURIComponent(state.pass)}/${r.itemId}.${r.ext||'mp4'}`];startPlayer({type:r.type,title:r.name,icon:r.icon,itemId:String(r.itemId),ext:r.ext||'mp4',urls,seekable:true,resumeKey:key,start:r.time})};g.appendChild(b)})}
async function renderFavorites(){activateTab('favorites');const c=$('mainContent');c.innerHTML='<div class="section-head"><h2>Favorites</h2></div><div class="loading">Loading…</div>';try{const sections=[];for(const type of ['LIVE','VOD','SERIES']){const ids=state.favorites[type]||[];if(!ids.length)continue;const items=await getStreams(type,'0');sections.push([type,items.filter(x=>ids.includes(itemId(type,x)))])}c.innerHTML='';if(!sections.length){c.innerHTML='<div class="empty">No favorites yet</div>';return}sections.forEach(([type,items])=>{const h=document.createElement('div');h.className='section-head';h.innerHTML=`<h2>${type==='LIVE'?'Live':type==='VOD'?'Movies':'Series'}</h2>`;const g=document.createElement('div');g.className='grid';g.id='fav-'+type;c.append(h,g);items.forEach(it=>{const b=document.createElement('button');b.className='card '+(type==='LIVE'?'live-card':'');b.innerHTML=`<div class="poster">${itemPoster(it)?`<img src="${escapeHtml(itemPoster(it))}" alt="">`:'<div class="fallback-logo">A</div>'}</div><div class="name">${escapeHtml(itemName(it))}</div>`;b.onclick=()=>type==='SERIES'?openSeries(it):playItem(type,it);g.appendChild(b)})})}catch(e){renderApiError(e,renderFavorites)}}
async function renderSearch(){activateTab('search');const c=$('mainContent');c.innerHTML='<div class="section-head"><h2>Search</h2></div><div class="searchbox"><input id="globalSearch" type="search" placeholder="Search live, movies and series..."></div><div id="searchResults" class="empty">Type at least 2 characters</div>';let t;$('globalSearch').oninput=e=>{clearTimeout(t);t=setTimeout(()=>globalSearch(e.target.value),300)}}
async function globalSearch(q){q=q.trim().toLowerCase();const host=$('searchResults');if(q.length<2){host.className='empty';host.innerHTML='Type at least 2 characters';return}host.className='loading';host.textContent='Searching…';try{const all=await Promise.all(['LIVE','VOD','SERIES'].map(t=>getStreams(t,'0')));host.className='';host.innerHTML='';['LIVE','VOD','SERIES'].forEach((type,i)=>{const matches=all[i].filter(x=>itemName(x).toLowerCase().includes(q)).slice(0,80);if(!matches.length)return;const h=document.createElement('div');h.className='section-head';h.innerHTML=`<h2>${type==='LIVE'?'Live':type==='VOD'?'Movies':'Series'}</h2>`;const g=document.createElement('div');g.className='grid';host.append(h,g);matches.forEach(it=>{const b=document.createElement('button');b.className='card '+(type==='LIVE'?'live-card':'');b.innerHTML=`<div class="poster">${itemPoster(it)?`<img src="${escapeHtml(itemPoster(it))}" alt="">`:'<div class="fallback-logo">A</div>'}</div><div class="name">${escapeHtml(itemName(it))}</div>`;b.onclick=()=>type==='SERIES'?openSeries(it):playItem(type,it);g.appendChild(b)})});if(!host.children.length){host.className='empty';host.textContent='No results'}}catch(e){renderApiError(e,()=>globalSearch(q))}}
function renderApiError(e,retry){const c=$('mainContent');const hint=String(e).includes('Failed to fetch')&&!state.proxy?'<p>If this server blocks browser requests, deploy the included free Cloudflare Worker and paste its URL in Connection settings.</p>':'';c.innerHTML=`<div class="empty"><h3>Connection error</h3><p>${escapeHtml(e.message||String(e))}</p>${hint}<button class="secondary" id="retryApi">Retry</button></div>`;$('retryApi').onclick=retry}

function showSheet(title,options,onPick){$('sheetTitle').textContent=title;const b=$('sheetBody');b.innerHTML='';options.forEach((o,i)=>{const x=document.createElement('button');x.className='sheet-option'+(o.active?' active':'');x.textContent=o.label;x.onclick=()=>{onPick(o,i);closeSheet()};b.appendChild(x)});$('sheet').classList.remove('hidden')}
...
$('refreshBtn').onclick=()=>{state.categories={LIVE:null,VOD:null,SERIES:null};state.streams.clear();state.seriesInfo.clear();toast('Cache cleared');renderHome()};
...
$('playerBack').onclick=closePlayer;$('playPauseBtn').onclick=()=>{const v=$('video');v.paused?v.play():v.pause();showControls()};$('scaleBtn').onclick=cycleScale;$('audioBtn').onclick=openAudio;$('subBtn').onclick=openSubs;$('pipBtn').onclick=togglePip;$('retryBtn').onclick=()=>loadSource(state.sourceIndex,$('video').currentTime||0);$('seekBar').oninput=e=>{const v=$('video');if(Number.isFinite(v.duration)&&v.duration>0)v.currentTime=(Number(e.target.value)/1000)*v.duration;showControls()};$('sheetClose').onclick=closeSheet;$('sheet').onclick=e=>{if(e.target===$('sheet'))closeSheet()};

addEventListener('online',()=>{if(state.playerTarget){toast('Internet restored');loadSource(state.sourceIndex,$('video').currentTime||0)}});addEventListener('offline',()=>{if(state.playerTarget){$('playerStatus').textContent='Offline — waiting for network';showControls()}});
